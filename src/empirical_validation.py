...
# python3 /cephyr/users/luque/Alvis/DCMO/src/empirical_validation.py --scenic /cephyr/users/luque/Alvis/DCMO/src/safety_program.scenic --num_steps 300 --num_sim 100 --results_dir /mimer/NOBACKUP/groups/naiss2024-22-1336/DCMO_alj/test_empirical_validation  --controllers_folder /mimer/NOBACKUP/groups/naiss2025-22-1298/CMO/experiments/checkpoints/train_faster 
...
if __name__ == "__main__":
	...
	## arguments 
	parser.add_argument('--num_sim', help='number of simulations',type=int,default=2000)
	...
	controllers_contexts_space = list(itertools.product(controllers, contexts))
	len_space = len(controllers_contexts_space)
      
	df = pd.DataFrame(columns=["controller", "weather", "dist", "speed", "rew_sta", "rew_eff", "rew_safe"])
	...
	for i in range(NUM_SIM):
		...
		j = i % len_space
		[cont, [weather, dist, speed]] = controllers_contexts_space[j]
		...
		while True:
			try: 
				...
				os.system(f"scenic -S {SCENIC} --count 1 --time {NUM_STEPS} --2d --param controller_path {cont} --param weather {weather} --param results_path {modd_dir} --param dist_car {dist} --param speed_car {speed} --param render 0")  
				# print("Simulation worked", flush=True)

				r_sta, r_eff, r_safe, _ = get_reward(results_path=modd_dir, controller_path=cont)
				# print("Computed rewards", flush=True)
				df.loc[i] = [cont, weather, dist, speed, r_sta, r_eff, r_safe]
