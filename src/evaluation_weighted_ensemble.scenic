...
WEIGHTS_PATH = globalParameters.weights
    ...
    while True:
        ...
        if isinstance(front_img, np.ndarray):
            ...
            for i in range(len(self.controllers)):
                result = predict_single(self.controllers[i], input_img, self.select_maneuver, device, transform, 100.0) 
                results += [result]

            cte_pred = np.sum(np.array([e["cte"] for e in results])*self.weights_ensemble)
            dist_pred = np.sum(np.array([e["distance_m"] for e in results])*self.weights_ensemble)
...
behavior EgoBehavior(target_speed = 10, leaderCar=None):
    ...
    if self.sensor_created_flag:
        ...
        with open(WEIGHTS_PATH, "rb") as f: 
            self.weights_ensemble = np.load(f) 
...
if dist_to_leader <= 15:
    ...
    ego = new Car following roadDirection from car1.position for -1*DIST_CAR1,
        ...
        with weights_ensemble np.zeros(9),
