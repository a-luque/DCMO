...
# python3 /cephyr/users/luque/Alvis/DCMO/src/evaluation_weighted_ensemble.py --scenic /cephyr/users/luque/Alvis/DCMO/src/evaluation_weighted_ensemble.scenic --num_steps 300 --num_sim 500 --results_dir /mimer/NOBACKUP/groups/naiss2024-22-1336/DCMO_alj/testing_weighted_ensemble  --controllers_folder /mimer/NOBACKUP/groups/naiss2025-22-1298/CMO/experiments/checkpoints/train_faster --weights_path /mimer/NOBACKUP/groups/naiss2024-22-1336/DCMO_alj/weights_ensemble/weights_avg_ensemble.npy
    ...
    for i in range(NUM_SIM):
        ...
        while True:
            try: 
                ...
                os.system(f"scenic -S {SCENIC} --count 1 --time {NUM_STEPS} --2d -s {seed} --param controllers_dir {CONTROLLERS_FOLDER} --param weather {weather} --param results_path {modd_dir} --param dist_car {dist} --param speed_car {speed} --param render 0 --param weights {WEIGHTS_PATH}")  
