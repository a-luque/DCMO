# os.system(f"scenic -S evaluation_dcmo.scenic --count 1 --time 300 --2d --param controllers_dir "/mimer/NOBACKUP/groups/naiss2025-22-1298/CMO/experiments/checkpoints/train_faster/" --param weather 'ClearNoon' --param results_path "test_controller_sim_results/0" --param dist_car 30 --param speed_car 4 --param dcmo_path /mimer/NOBACKUP/groups/naiss2025-22-1298/CMO/experiments/bandit_checkpoint_e_s_snapshots/snapshot_t5000.npz" --param contexts_order /cephyr/users/luque/Alvis/DCMO/stage2/context_cells.csv --param eff_first 1)
...
DCMO_PATH = globalParameters.dcmo_path
PI_SAFE_PATH = globalParameters.pi_safe_path
RULEBOOK = globalParameters.rulebook_path
CONTEXTS_ORDER = globalParameters.contexts_order
EFF_FIRST = int(globalParameters.eff_first)


contexts_order = pd.read_csv(CONTEXTS_ORDER, index_col=0)
...
behavior ControllerBehavior(target_speed = 10, controller_path = None, leaderCar = None):
    ...
    while True:
        ...
        if isinstance(front_img, np.ndarray):
            ...
            context_index = int(contexts_order.index[(contexts_order["weather"] == globalParameters.weather) & (contexts_order["distance"] == distance_bucket) & (contexts_order["speed"] == speed_bucket)][0])
            ...
            for c_index in safe_controllers:
                rew_estimation = self.dcmo["mu_hat"][c_index, context_index, :]
                if EFF_FIRST:
                    weights_rulebook = np.array([float(w_eff), float(w_sta)])
                else:
                    weights_rulebook = np.array([float(w_sta), float(w_eff)])

                rulebook_reward = np.dot(rew_estimation, weights_rulebook.T)
