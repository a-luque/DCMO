...
# python3 /cephyr/users/luque/Alvis/DCMO/src/evaluation_dcmo.py --scenic /cephyr/users/luque/Alvis/DCMO/src/evaluation_dcmo.scenic --num_steps 300 --num_sim 500 --results_dir /mimer/NOBACKUP/groups/naiss2024-22-1336/DCMO_alj/testing_dcmo  --moe_path /mimer/NOBACKUP/groups/naiss2025-22-1298/CMO/experiments/checkpoints/train_faster 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dcmo_path", type=str, default="")
    parser.add_argument("--rulebook_path", type=str, default="")
    parser.add_argument("--pi_safe_path", type=str, default="")
    parser.add_argument("--contexts_order", type=str, default="/cephyr/users/luque/Alvis/DCMO/stage2/context_cells.csv")
    parser.add_argument('--eff_first', type=int,default=1)
    ...
    SCENIC = args.scenic
    DCMO_PATH = args.dcmo_path
    RULEBOOK_PATH = args.rulebook_path
    PI_SAFE_PATH = args.pi_safe_path
    EFF_FIRST = args.eff_first
    CONTEXTS_ORDER = args.contexts_order
    ...
    for i in range(NUM_SIM):
        ...
        while True:
            try: 
                ...
                os.system(f"scenic -S {SCENIC} --count 1 --time {NUM_STEPS} --2d -s {seed} --param controllers_dir {CONTROLLERS_FOLDER} --param dcmo_path {DCMO_PATH} --param rulebook_path {RULEBOOK_PATH} --param pi_safe_path {PI_SAFE_PATH} --param weather {weather} --param results_path {modd_dir} --param dist_car {dist} --param speed_car {speed} --param render 0 --param eff_first {EFF_FIRST} --param contexts_order {CONTEXTS_ORDER}")  
