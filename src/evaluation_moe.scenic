...
behavior ControllerBehavior(target_speed = 10, controller_path = None, leaderCar = None):
    ...
    while True:
        ...
        if isinstance(front_img, np.ndarray):
            ...
            #m     = torch.tensor([self.select_maneuver], dtype=torch.long).to(device)
            #c     = torch.cat([torch.tensor(self.weather), torch.tensor(distance_bucket), torch.tensor(speed)]).to(device)

            prediction = self.moe.predict_single(Image.fromarray(front_img), self.select_maneuver, self.weather, distance_bucket, speed, "cuda", transform, 100.0) 
            # prediction = model.predict(x, m, c)
...
behavior EgoBehavior(target_speed = 10, leaderCar=None):
    ...
    if self.sensor_created_flag:
        ...
        self.moe = MoE(
            controllers_dir=CONTROLLERS_FOLDER,
        )
        self.moe.cuda()

        if hasattr(torch, "compile"):
            print("torch.compile() enabled", flush=True)
            self.moe = torch.compile(self.moe)
        checkpoint = torch.load(MOE_PATH, map_location="cuda")
        self.moe.load_state_dict(checkpoint["model_state"])
