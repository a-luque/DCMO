...
def run_fixed_context_eval(
    ...
    for i in range(start, n_runs):
        while True:
            ...
            try:
                r, safety_info = simulate(sim_seed, cell, controller, save_path, order="es", t=i)
                break
...
def run_all_controllers(
    ...
    for controller in controllers:
        ...
        checkpoint_path = os.path.join(
            checkpoint_dir, f"testresults_rq0_{cell_str}_{controller}.npz"
        )
...
def simulate(sim_seed, cell, controller_path: str, save_path: str, order: str, t: int) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    base_save_path = f"test_results_all/{cell_str}"
