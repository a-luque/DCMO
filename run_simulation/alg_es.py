...
class ContextualKObjectiveBandit:
    ...
    def run(
        ...
        for t in range(start_t + 1, self.T + 1):                     # line 5
            ...
            while True:
                sim_seed = np.random.randint(1, 2**32 - 1)  # generate a random seed for each simulation
                try:
                    r_t, safety_info = simulate(cell, sim_seed, controller_path, save_path, order, t) # shape (K,)
                    break
                except Exception as e:
                    print(f"Simulation failed for cell {cell}:", e)
                    time.sleep(5)
...
def get_reward(results_path: str, order: str, timestep: float = 0.1, tau: float = 1.0) -> list:
    ...
    lane_invasion_count = int((np.absolute(cte_vals["values"])>0.7).sum())
    ...
    if lane_invasion_count > 30:
        lane_invasion = 1
    else: 
        lane_invasion = 0

    if near_collision or lane_invasion:
        safety_violation = 1
    else:
        safety_violation = 0
...
def simulate(cell, sim_seed: int, controller_path: str, save_path: str, order: str, t: int) -> np.ndarray:
    current_file_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    os.system(
        f"scenic -S {scenic_file_path} --seed {sim_seed} --count 1 --time 300 --2d "
...
if __name__ == "__main__":
    ...
    file_name = "main_alg_sim"
    order = "es"
    #order = "se"
    saved_file_path = f"{file_name}_{order}"

    bandit = ContextualKObjectiveBandit(
        T=T, ctx=ctx, beta=beta, eps=eps, K=K,
        n_controllers=n_controllers, pi_safe=pi,
        checkpoint_path=f"{saved_file_path}.npz",
        #checkpoint_path="bandit_checkpoint_sim7_se.npz",
    ...
    history = bandit.run(order, saved_file_path, simulate, start_t=start_t)
