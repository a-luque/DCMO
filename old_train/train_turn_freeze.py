"""
Data pipeline + training & evaluation for multi-task Distance+CTE CNNs
=======================================================================
Data layout (per simulation run):
    simulation_results/{0..499}/
        dist.npz          — real-valued distances (meters)
        cte.npz           — real-valued CTE (meters)
        maneuver.npz      — selected maneuver labels
        img/front_rgb_{t:.1f}.jpg  — front RGB frames

Usage:
    python train.py --granularity fine --backbone resnet50
    python train.py --granularity coarse --backbone resnet18 --epochs 30
"""
...
def train(
    ...
    Two-phase training:
      Phase 1 (epochs 1 .. warmup_epochs):
        Backbone frozen — only heads + shared layer train.
        Protects pretrained weights from noisy gradients at the start.
      Phase 2 (epochs warmup_epochs+1 .. epochs):
        Backbone unfrozen — full network fine-tuned at a lower lr.
        Backbone lr is 10x smaller than heads (set in main()).
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"cte_mae={val_metrics['cte_mae']:.4f}m "
...
def main():
    ...
    parser.add_argument("--warmup_epochs", type=int,   default=5,
                        help="Epochs to train with backbone frozen before full fine-tuning")
    ...
    parser.add_argument("--total_runs",  type=int,   default=700)
    ...
    # CTE is a steering angle in radians (range approx -1.2 to +1.2).
    # Override Huber delta to 0.1 rad — the L2->L1 transition point.
    # This makes the loss sensitive to small steering errors near 0
    # while being robust to the occasional large correction.
    cte_huber_delta = 0.1
