...
def simulate(cell, controller_path: str, t: int) -> np.ndarray:
    ...
    # results_dir = os.path.join(current_file_dir, f"sim_results/{t}/") # for order: e,s
...
if __name__ == "__main__":
    ...
    with open("context_cells.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["index", "weather", "distance", "speed"])

        for i, cell in enumerate(ctx.cells):
            writer.writerow([i, *cell])

    print("Saved context_cells.csv")
    """
    ...
    bandit = ContextualKObjectiveBandit(
        ...
        checkpoint_path="bandit_checkpoint_s_e.npz",
